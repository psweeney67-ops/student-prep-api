<?php
// Intelligence Briefing API - Front Controller (Simplified)
// This script handles incoming API requests, validates them,
// and schedules the background worker to run after the response is sent.
...
try {
    switch ($action) {
        case 'create-briefing':
            ...
            // --- NEW RELIABLE BACKGROUND PROCESS ---
            // We register a "shutdown function". This tells PHP to run our worker
            // AFTER it has sent the response and closed the connection to Lovable.
            // This is a robust way to handle background tasks without hanging.
            register_shutdown_function(function() use ($job_id) {
                // The command is simple: execute the worker script with the job ID.
                $command = "php worker.php " . escapeshellarg($job_id);
                // We don't need the output, just to run the command.
                shell_exec($command);
            });

            // Immediately return success to the client. The worker will start after this.

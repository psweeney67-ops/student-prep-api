...
// FIXED: File paths using absolute paths with __DIR__
$data_path = __DIR__ . '/data';
$jobs_path = $data_path . '/jobs';
$output_path_briefing = $data_path . '/briefing';
$output_path_podcast = $data_path . '/podcast';

// FIXED: Ensure all required directories exist
function ensure_directories($paths) {
    foreach ($paths as $path) {
        if (!is_dir($path)) {
            if (!mkdir($path, 0755, true)) {
                file_put_contents('php://stderr', "Failed to create directory: $path\n");
                return false;
            }
        }
    }
...
if (!ensure_directories([$data_path, $jobs_path, $output_path_briefing, $output_path_podcast])) {
    file_put_contents('php://stderr', "Failed to create required directories\n");
    exit(1);
}
...
/**
 * FIXED: Validate content before saving
 */
function validate_content($content, $min_length = 100) {
    if (empty($content) || strlen(trim($content)) < $min_length) {
        throw new Exception('Generated content appears to be too short or empty');
    }
    return true;
}
...
try {
    ...
    // PRESERVED: Your original research prompt
    ...
    // FIXED: Validate research content
    validate_content($factual_briefing_text, 200);
    ...
    // PRESERVED: Your original career coach prompt
    ...
    $career_insights = $career_coach_response['candidates'][0]['content']['parts'][0]['text'] ?? 'Error: Could not generate career insights.';
    
    // FIXED: Validate career insights
    validate_content($career_insights, 50);
    
    $final_briefing_text = $factual_briefing_text . "\n\n" . $career_insights;
    
    // FIXED: Validate final content
    validate_content($final_briefing_text, 300);
    ...
    $briefing_file_path = $output_path_briefing . '/' . $briefing_filename;
    
    // FIXED: Use LOCK_EX for safe file writing
    $saved = file_put_contents($briefing_file_path, $final_briefing_text, LOCK_EX);
    if ($saved === false) {
        throw new Exception('Failed to save briefing file to disk');
    }
    
    // CRITICAL: Mark briefing as complete immediately - don't wait for podcast
    $briefing_results = [
        'briefing_url' => '/briefing/' . $briefing_filename,  
        'briefing_content' => $final_briefing_text, // FIXED: Include full content, not truncated
    ...
    if (isset($job_data['include_podcast']) && $job_data['include_podcast']) {
        // Note: We don't change status back to processing - podcast runs in background
        
        // --- STEP 3: PODCAST GENERATION ---
        // PRESERVED: Your original podcast prompt
        ...
        // FIXED: Use LOCK_EX for safe file writing
        if (file_put_contents($raw_audio_path, $raw_audio_data, LOCK_EX) === false) {
            throw new Exception("Failed to save raw audio data");
        }
        ...
        // 4. Add podcast to existing results (don't overwrite)

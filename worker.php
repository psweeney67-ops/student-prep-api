...
// It uses a robust, two-phase process for each section with granular timeouts to prevent stalling.
...
function call_gemini_api(string $api_key, array $payload, string $model): array {
    // --- FIX: Added specific, granular timeouts to prevent stalling ---
    $client = new Client([
        'timeout'         => 300.0, // Total request time
        'connect_timeout' => 10.0,  // Time to establish connection
        'read_timeout'    => 290.0  // Time to wait for data after connecting
    ]);

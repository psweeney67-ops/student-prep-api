...
// It now uses a granular multi-step process for reliability.
...
function update_job_status(string $path, string $status, ?array $results = null, ?string $error = null): void {
    ...
    if ($results) {
        // Use array_merge to add new results without overwriting existing ones.
...
try {
    ...
    $sections = [
        'Company Overview' => "Provide a concise overview of '$company_name'.",
        'Strategic Response to Trends' => "Analyze how '$company_name' is strategically responding to the following industry trends: $trends_string. Use your search tool to find specific examples, products, or statements.",
        'Key Leadership' => "Identify the key C-suite executives at '$company_name' (CEO, CFO, CTO, etc.) and provide a brief summary for each.",
        'Competitive Landscape' => "Identify the main competitors of '$company_name' and briefly describe their market position.",
        'Recent News & Developments' => "Summarize 3-5 of the most significant recent news articles or developments concerning '$company_name'. Cite your sources."
    ];
    ...
    foreach ($sections as $title => $prompt) {
        ...
        $section_content = $section_response['candidates'][0]['content']['parts'][0]['text'] ?? "Could not generate content for this section.";
    ...
    if (isset($job_data['include_podcast']) && $job_data['include_podcast']) {
        // (Podcast logic remains the same as the previous version)

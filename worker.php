...
// --- FINAL FIX: Increase memory limit to prevent silent crashes ---
...
function call_gemini_api(string $api_key, array $payload, string $model): array {
    $client = new Client([
        'timeout' => 270.0, // A safe timeout (4.5 minutes)
        'connect_timeout' => 20.0, // Allow more time to connect
        'read_timeout' => 270.0,
    ]);
...
try {
    ...
    $sections = [
        'Company Overview' => "Find highly relevant sources to write a 2-3 paragraph overview of '$company_name'. Prioritize the official company website, its Wikipedia page, and a recent high-level news article about its business.",
    ...
    // --- STEP 3: CAREER COACH ---
    // (Career Coach and subsequent steps remain the same)
    // ...

...
// It now uses aggressively scoped prompts with token limits and a retry mechanism for maximum reliability.
...
try {
        ...
        for ($retry = 0; $retry < $max_retries; $retry++) {
            try {
                $section_payload = [
                    ...
                    'tools' => [['google_search' => new stdClass()]],
                    'generationConfig' => [
                        'maxOutputTokens' => 1024 // Hard limit on response length
                    ]
                ];
    ...
    $career_coach_payload = [
        ...
        'generationConfig' => [
            'maxOutputTokens' => 1024 // Also limit the coach's output
        ]
    ];

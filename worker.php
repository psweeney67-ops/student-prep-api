...
function call_gemini_api(string $api_key, array $payload, string $model = 'gemini-2.5-flash-preview-05-20'): array {
    ...
    // --- ADDED FROM GEMINI'S SUGGESTION ---
    // Add safety settings to reduce the chance of truncation for non-harmful content.
...
/**
 * Validates content to ensure it meets minimum requirements.
 * @param string $content The content to validate.
 * @param int $min_length Minimum length required.
 * @return bool True if content is valid, false otherwise.
 */
function validate_content($content, $min_length = 100): bool {
    if (empty($content)) {
        return false;
    }
...
try {
    ...
    // Goal: Identify trends, synthesize the factual part of the briefing, with better error handling
    
    $company_name = $job_data['company_name'];
    
    // Simplified approach - don't ask for JSON when using search tools
    $research_prompt = "Act as a research analyst creating a comprehensive market briefing for '$company_name'. 

Create a detailed briefing with these sections:
1. Company Overview
2. Industry Position & Market Trends  
3. Strategic Response to Current Trends
4. Key Leadership & People
5. Competitive Landscape
6. Recent News & Developments

Use Google Search to find at least 8-10 high-quality, recent sources. Cite all sources clearly. Write in professional markdown format with clear headings and bullet points.";
    ...
    $factual_briefing_text = $research_response['candidates'][0]['content']['parts'][0]['text'] ?? '';
    
    // Validate research content
    if (!validate_content($factual_briefing_text, 500)) {
        throw new Exception("Research content too short or invalid: " . strlen($factual_briefing_text) . " characters");
    }
    
    // Create a simple research log based on successful completion
    $research_log = [
        "Initiated comprehensive research for $company_name",
        "Conducting Google searches for company information and market data",
        "Analyzing industry trends and competitive positioning", 
        "Gathering information on leadership team and recent developments",
        "Synthesizing research findings into structured briefing",
    ...
    // Validate final briefing
    if (!validate_content($final_briefing_text, 1000)) {
        throw new Exception("Final briefing content too short: " . strlen($final_briefing_text) . " characters");
    }
